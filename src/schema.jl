...
process_sql(s::String) =
    let nothingorcapture(x, idx=2) = isnothing(x) ? nothing : x.captures[idx]
        ...
        fkeys(s) = Dict(map(x -> x.captures[2] => (x.captures[4], x.captures[6]),
            eachmatch(r"(FOREIGN\s+KEY\s*\(\s*\[?) (\w+) (\]?\s*\)\s*REFERENCES\s+\"?) (\w+) (\"?\s*\(\s*\[?) (\w+) (\]?\s*\)\s*) (\s|$|,|\))"x, s)))  # TODO: is xm here for linebreak not \s in last clause better?
...
get_sqls(connection::DBInterface.Connection, dialectname::Symbol=SQLDialect(typeof(conn)).name) =
    [x.sql for x in DBInterface.execute(connection, "SELECT * FROM sqlite_schema") if !ismissing(x.sql)]
